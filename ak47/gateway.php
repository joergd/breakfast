...
//
// $Id: gateway.php,v 1.12 2004/03/30 00:25:40 khlee Exp $
...
$keydata = 'a:3:{s:7:"keysize";i:1024;s:1:"e";s:5:"65537";s:1:"n";s:309:"";}';


// set the public key
$pubkey = unserialize($keydata);
...
header('X-BITPASS-GW-VER: PHP $Revision: 1.12 $', FALSE);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_GET['_bp_']) {
    if ($_POST['BITPASS_VOUCHER_SUBMISSION']) {
        // check the voucher 
        ...
        exit;
    } else if ($_POST['BITPASS_DIAGNOSIS_REQUEST']) {
...
function fulfill_req($base, $path_info, $validFor) {
            ...
            if (is_file($fname)) {
                if (ereg("\.([^.]+)$", $fragment, $ext)) {
                    if (in_array($ext[1], $GLOBALS['PHP_EXT'])) {
                        ...
                        $_SERVER['PATH_INFO'] = substr($_SERVER['PHP_SELF'], strlen($script_name));
			return $fname;
                    }
                }
            }
